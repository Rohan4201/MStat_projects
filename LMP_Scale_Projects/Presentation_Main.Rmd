---
title: "LMP Rank Tests for Scale Shift"
...
- Report score function using $\widehat{\mathbb{E}}[g(X_{(i)})] = \frac{1}{5000} \sum_{r=1}^{5000} g(\widehat{X_{(i,r)}})$
...
- For Normal Population, $X_i \overset{iid}\sim N(0, \sigma_x^2); \ Y_j \overset{iid}\sim N(0, \sigma_y^2)$
...
Distribution under Null: $\theta = 1$
...
## LMP Scale for Weibull

Let's work with Weibull(k,1) Distribution

$$h(x) = kx^{k-1}e^{-x^k} ; \ \ x>0$$

Consider independent samples, 
$$X_i \overset{iid}\sim Weibull(k,\theta); \ \ \ Y_j^k \overset{iid}\sim Weibull(k,1)$$

Under $H_0$, by making the transformation:
$$X_i^k \overset{iid}\sim Exp(1); \ \ \ Y_j^k \overset{iid}\sim Exp(1)$$

$$\text{Savage Statistic should suffice as LMP Statistic}$$
$$\text{As, power transformation causes no change to rank vector.}$$
---

## Weibull Distribution LMP: Power Curve

```{r, fig.align='center', out.height="90%", out.width="90%"}
knitr::include_graphics("Image\\gs_weibull_power_curve_changed.jpeg")

```

---

#LMP Scale for Cauchy
Let's work with Cauchy(0,1) Distribution:
$$h(x) = \frac{1}{\pi (1 + x^2)}$$

Score function:

$$\frac{f'(X_{(i)},0)}{f'(X_{(i)},0)} = 1 - \frac{2 X_{(i)}}{1 + X_{(i)}^2}$$

LMP Test for $H_0: \theta = 1 \text{ vs } \ \theta > 1$

$$\sum_{i=1}^n - \ \mathbb{E} \left[ \frac{1}{1 + X_{(i)}^2}\right] > k_{\alpha}$$
--

$$\text{Gupta-Shinde Statistic ?}$$
---

## Cauchy Distribution LMP: Power Curve

```{r, fig.align='center', out.height="90%", out.width="90%"}
knitr::include_graphics("Image\\gs_cauchy_power_curve_changed.jpeg")
...
### 1. Modified Wilcoxon Rank Sum(WRS*):
- <font size="3"> Yes! Wilcoxon Rank Sum test can also be used to test the difference in scale parameters of two populations. (Reject the test for larger values of WRS) </font>
<font size="2"> $$WRS^*= \sum_{1=1}^n R_{(i,N)}^*$$ </font>
...
---
## Modified WRS under different populations
